...
async function main() {
  ...
  for (let i = 0; i < 1000; i++) {
    ...
    const orderNumber = `ORD${faker.number.int({ min: 10000, max: 99999 })}`;
    const model = faker.helpers.arrayElement(models);
    const serialNumber = faker.string.alphanumeric(10);
    const faultDescription = faker.helpers.arrayElement([
    ...
    const itemsOrdered = faker.helpers.arrayElement([
      "Batteri til bærbar",
      "Skærm til laptop",
      "Tastatur",
      "RAM-modul 16GB DDR4",
    ...
    const supplier = faker.helpers.arrayElement([
      "IT-eksperten",
      "Nordic Data",
      "ElektronikPartner",
      "PC Service Danmark",
  ...
  ordersToInsert.forEach(order => {
    ...
    for (let i = 0; i < numHistory; i++) {
      const newStatus = faker.helpers.arrayElement(statuses.filter(s => s !== prevStatus));
      const date = faker.date.between({ from: order.orderDate, to: new Date() });
