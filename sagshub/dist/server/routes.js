...
const cases_1 = require("./src/routes/cases");
...
const updateStatusSchema = zod_1.z.object({
    ...
    comment: zod_1.z.string().min(1, "Kommentar er påkrævet"),
    updatedByName: zod_1.z.string().optional(),
});
...
async function generateCaseNumber(treatment) {
    ...
    try {
        ...
        const cases = await storage_1.storage.getLatestCaseNumber(prefix);
...
async function registerRoutes(app) {
    ...
    app.get("/api/internal-cases/unread-count", async (req, res) => {
        ...
        try {
            ...
            try {
                const count = await storage_1.storage.getUnreadInternalCasesCount(userId);
    ...
    app.get("/api/customers", async (req, res) => {
        ...
        const customers = await storage_1.storage.getPaginatedCustomers(page, pageSize, searchTerm);
    ...
    app.get("/api/customers/search", async (req, res) => {
        ...
        try {
            ...
            if (!searchTerm) {
                ...
                const allCustomers = await storage_1.storage.getCustomers();
                return res.json(allCustomers);
            }
            const customers = await storage_1.storage.searchCustomers(searchTerm);
    ...
    app.get("/api/customers/:id", async (req, res) => {
        if (!req.isAuthenticated() || !req.user.isWorker)
            return res.sendStatus(403);
        const customer = await storage_1.storage.getCustomer(parseInt(req.params.id));
        if (!customer)
    ...
    app.post("/api/customers", async (req, res) => {
        if (!req.isAuthenticated() || !req.user.isWorker)
            return res.sendStatus(403);
        const result = schema_1.insertCustomerSchema.safeParse(req.body);
        if (!result.success) {
            return res.status(400).json(result.error);
        }
        const customer = await storage_1.storage.createCustomer(result.data);
        res.status(201).json(customer);
    });
    app.patch("/api/customers/:id", async (req, res) => {
        if (!req.isAuthenticated() || (!req.user.isWorker && !req.user.isAdmin))
            return res.sendStatus(403);
        const result = schema_1.insertCustomerSchema.partial().safeParse(req.body);
        if (!result.success) {
            return res.status(400).json(result.error);
        }
        const customer = await storage_1.storage.updateCustomer(parseInt(req.params.id), result.data);
        if (!customer)
            return res.sendStatus(404);
        res.json(customer);
    });
    ...
    app.get("/api/customers/:id/cases", async (req, res) => {
        ...
        try {
            ...
            const cases = await storage_1.storage.getCases(customerId);
            res.json(cases);
        }
        catch (error) {
    ...
    app.get("/api/customers/:id/rma", async (req, res) => {
        ...
        try {
            ...
            const rmas = await storage_1.storage.getRMAsByCustomerId(customerId);
            res.json(rmas);
        }
        catch (error) {
    ...
    app.get("/api/search", async (req, res) => {
        ...
        try {
            ...
            const [customers, cases, rmas, orders] = await Promise.all([
                storage_1.storage.searchCustomers(searchTerm).catch((err) => { console.error("Customer search error:", err); return []; }),
                storage_1.storage.searchCases(searchTerm).catch((err) => { console.error("Case search error:", err); return []; }),
                storage_1.storage.searchRMAs(searchTerm).catch((err) => { console.error("RMA search error:", err); return []; }),
                storage_1.storage.searchOrders(searchTerm).catch((err) => { console.error("Order search error:", err); return []; })
            ]);
            ...
            const results = [
                ...
                ...orders.map(order => ({
                    id: order.id,
                    type: 'order',
                    title: `${order.orderNumber} - ${order.title || 'Bestilling'}`,
    ...
    app.post("/api/users", async (req, res) => {
        ...
        try {
            const existingUser = await storage_1.storage.getUserByUsername(result.data.username);
            if (existingUser) {
                return res.status(400).json({ error: "Brugernavn findes allerede" });
            }
            const hashedPassword = await (0, auth_1.hashPassword)(result.data.password);
    ...
    app.delete("/api/users/:id", async (req, res) => {
        ...
        try {
            await storage_1.storage.deleteUser(userId);
    ...
    app.get("/api/cases", async (req, res) => {
        ...
        try {
            const cases = await storage_1.storage.getPaginatedCases({
                page,
                pageSize,
                searchTerm,
                treatment,
    ...
    app.get("/api/cases/alarm", async (req, res) => {
        ...
        try {
            const alarmCases = await storage_1.storage.getAlarmCases();
    ...
    app.get("/api/cases/status-counts", async (req, res) => {
        ...
        try {
            ...
            const statusCounts = await storage_1.storage.getStatusCounts();
    ...
    app.get("/api/cases/total", async (req, res) => {
        ...
        try {
            const total = await storage_1.storage.getTotalCases();
    ...
    app.patch("/api/cases/:id(\\d+)", async (req, res) => {
        ...
        try {
            ...
            const result = schema_1.insertCaseSchema.partial().safeParse(req.body);
            if (!result.success) {
            ...
            const updatedCase = await storage_1.storage.updateCase(caseId, result.data);
            if (!updatedCase) {
    ...
    app.get("/api/cases/:idOrNumber", async (req, res) => {
        try {
            ...
            let case_ = await storage_1.storage.getCase(idOrNumber);
            if (!case_ && !isNaN(Number(idOrNumber))) {
                case_ = await storage_1.storage.getCase(Number(idOrNumber));
            }
            if (!case_) {
    ...
    app.post("/api/cases", async (req, res) => {
        if (!req.isAuthenticated() || !req.user.isWorker)
            return res.sendStatus(403);
        const result = schema_1.insertCaseSchema.safeParse(req.body);
        if (!result.success) {
            return res.status(400).json(result.error);
        }
        try {
            ...
            if (!customerId && result.data.customerPhone) {
                const newCustomer = await storage_1.storage.createCustomer({
            ...
            const case_ = await storage_1.storage.createCase(caseDataToCreate);
    ...
    app.post("/api/cases/:id/status", async (req, res) => {
        ...
        try {
            ...
            const updatedCase = await storage_1.storage.updateCaseStatusWithHistory(caseId, status, comment || "", req.user.id, updatedByName_);
            res.json(updatedCase);
        }
        catch (error) {
    ...
    app.get("/api/cases/:id/status-history", async (req, res) => {
        if (!req.isAuthenticated())
            return res.sendStatus(401);
        const history = await storage_1.storage.getCaseStatusHistory(parseInt(req.params.id));
        res.json(history);
    });
    // RMA routes
    app.get("/api/rma", async (req, res) => {
        ...
        try {
            ...
            const rmas = await storage_1.storage.getPaginatedRMAs(options);
    ...
    app.get("/api/rma/:id", async (req, res) => {
        if (!req.isAuthenticated() || !req.user.isWorker)
            return res.sendStatus(403);
        try {
            const rma = await storage_1.storage.getRMA(parseInt(req.params.id));
            if (!rma)
    ...
    app.post("/api/rma", async (req, res) => {
        if (!req.isAuthenticated() || !req.user.isWorker)
            return res.sendStatus(403);
        const result = schema_1.insertRMASchema.safeParse(req.body);
        if (!result.success) {
            return res.status(400).json(result.error);
        }
        try {
            ...
            const rma = await storage_1.storage.createRMA(rmaData);
    ...
    app.get("/api/rma/:id/status-history", async (req, res) => {
        if (!req.isAuthenticated() || !req.user.isWorker)
            return res.sendStatus(403);
        try {
            const history = await storage_1.storage.getRMAStatusHistory(parseInt(req.params.id));
            res.json(history);
        }
        catch (error) {
    ...
    app.patch("/api/rma/:id/status", async (req, res) => {
        ...
        try {
            ...
            const rma = await storage_1.storage.updateRMAStatusWithHistory(parseInt(req.params.id), status, comment, req.user.id, updatedByName);
            res.json(rma);
        }
        catch (error) {
    ...
    app.patch("/api/rma/:id", async (req, res) => {
        ...
        try {
            ...
            const result = schema_1.insertRMASchema.partial().safeParse(req.body);
            if (!result.success) {
                return res.status(400).json(result.error);
            }
            const updatedRMA = await storage_1.storage.updateRMA(rmaId, result.data);
            if (!updatedRMA) {
                return res.status(404).json({ error: "RMA ikke fundet" });
            }
            res.json(updatedRMA);
    ...
    app.get("/api/orders", async (req, res) => {
        ...
        try {
            ...
            const orders = await storage_1.storage.getPaginatedOrders({
                page,
                pageSize,
                status,
                sort,
    ...
    app.post("/api/orders", async (req, res) => {
        ...
        try {
            ...
            let orderNumber;
            try {
                orderNumber = await (0, storage_1.generateOrderNumber)();
            ...
            const result = schema_1.insertOrderSchema.safeParse(orderData);
            if (!result.success) {
            ...
            try {
                const order = await storage_1.storage.createOrder(result.data);
    ...
    app.patch("/api/orders/:id", async (req, res) => {
        ...
        try {
            ...
            const result = schema_1.insertOrderSchema.partial().safeParse(req.body);
            if (!result.success) {
                return res.status(400).json(result.error);
            }
            const order = await storage_1.storage.updateOrder(orderId, result.data);
            if (!order) {
                return res.status(404).json({ error: "Bestilling ikke fundet" });
            }
            res.json(order);
    ...
    app.patch("/api/orders/:id/status", async (req, res) => {
        ...
        try {
            ...
            const order = await storage_1.storage.updateOrderStatus(orderId, status);
            if (!order) {
                return res.status(404).json({ error: "Bestilling ikke fundet" });
            }
            res.json(order);
    ...
    app.get("/api/customers/:id/orders", async (req, res) => {
        ...
        try {
            ...
            const orders = await storage_1.storage.getOrdersByCustomerId(customerId);
            res.json(orders);
        }
        catch (error) {
    ...
    app.get("/api/cases/:id/orders", async (req, res) => {
        ...
        try {
            ...
            const orders = await storage_1.storage.getOrdersByCaseId(caseId);
            res.json(orders);
        }
        catch (error) {
    ...
    app.get("/api/internal-cases", async (req, res) => {
        ...
        try {
            ...
            const userId = req.user.id;
            const result = await storage_1.storage.getPaginatedInternalCases({
                page,
                pageSize,
                userId,
                onlySent,
    ...
    app.patch("/api/internal-cases/:id/read", async (req, res) => {
        ...
        try {
            ...
            const [internalCase] = await db_1.db
                .select()
                .from(schema_2.internalCases)
                .where((0, drizzle_orm_1.eq)(schema_2.internalCases.id, id));
            if (!internalCase) {
    ...
    app.post("/api/internal-cases", async (req, res) => {
        ...
        try {
            ...
            const [newInternalCase] = await db_1.db
                .insert(schema_2.internalCases)
                .values({
                caseId,
    ...
    app.post("/api/temp/insert-internal-cases", async (req, res) => {
        try {
            ...
            const result1 = await storage_1.storage.createInternalCase({
                caseId: 34,
                senderId: 1,
                receiverId: 2,
            ...
            const result2 = await storage_1.storage.createInternalCase({
                caseId: 32,
                senderId: 3,
                receiverId: 2,
            ...
            const result3 = await storage_1.storage.createInternalCase({
                caseId: 29,
                senderId: 4,
                receiverId: 2,
    ...
    app.post("/api/temp/fix-finn-password", async (req, res) => {
        ...
        try {
            const finn = await storage_1.storage.getUserByUsername("Finn");
            if (!finn) {
                return res.status(404).json({ error: "Bruger ikke fundet" });
            }
            const hashedPassword = await (0, auth_1.hashPassword)("finnernice");
            await storage_1.storage.updateUserPassword(finn.id, hashedPassword);
    ...
    app.post("/api/cases/translate-to-danish", async (req, res) => {
        if (!req.isAuthenticated() || !req.user.isWorker)
            return res.sendStatus(403);
        try {
            const updatedCases = await (0, translate_1.findAndTranslateEnglishCases)();
            res.json({
    ...
    app.delete("/api/customers/:id", async (req, res) => {
        ...
        try {
            const customer = await storage_1.storage.getCustomer(customerId);
            if (!customer) {
                return res.status(404).json({ error: "Kunde ikke fundet" });
            }
            // Slet kunden
            await storage_1.storage.deleteCustomer(customerId);
    ...
    app.get("/api/customers/:id/export", async (req, res) => {
        ...
        try {
            ...
            if (includeCases) {
                const cases = await storage_1.storage.getCasesByCustomerId(customerId);
