...
const translate_1 = require("../server/translate");
...
async function main() {
    ...
    `);
        ...
        for (const case_ of result.rows) {
            const translatedDescription = (0, translate_1.translateText)(case_.description);
            const translatedNotes = case_.important_notes ? (0, translate_1.translateText)(case_.important_notes) : null;
            if (translatedDescription !== case_.description ||
                (translatedNotes && translatedNotes !== case_.important_notes)) {
                await pool.query(`UPDATE cases 
           SET description = $1, 
               important_notes = $2, 
               updated_at = NOW() 
           WHERE id = $3`, [translatedDescription, translatedNotes, case_.id]);
