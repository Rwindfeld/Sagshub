...
function genererDanskBy(): string {
  return faker.helpers.arrayElement(danskeByer);
}

function genererDanskFejlbeskrivelse(): string {
  const fejltyper = [
...
function genererDanskBehandling(): string {
...
async function generateCases(count: number) {
...
async function generateRMAs(customers: Customer[], cases: Case[], storage: IStorage) {
